...
- (id)init {
    self = [super init];
//    self.stunIP = @"stun.l.google.com";
//    self.stunIP =@"5.45.123.244";
    self.stunIP = kSTUNServerIP;
    
    STUNLog(@"STUN server: %@", self.stunIP);
...
- (void)requestPublicIPandPortWithUDPSocket:(GCDAsyncUdpSocket *)socket  andPort:(int) port delegate:(id<STUNClientDelegate>)_delegate{
    ...
    //
    [socket sendData:stunRequest toHost:self.stunIP port:kSTUNServerPort withTimeout:1.0 tag:1002];
}
...
- (void)sendIndicationMessage{
    ...
    //
    [udpSocket sendData:stunIndicationMessage toHost:self.stunIP port:kSTUNServerPort withTimeout:-1 tag:1003];
}
...
-(void)udpSocket:(GCDAsyncUdpSocket *)sock didNotConnect:(NSError *)error {
    STUNLog(@"STUN didNotConnect=%@", error);
}

-(void)udpSocket:(GCDAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error {
    STUNLog(@"STUN didNotSendDataWithTag=%@", error);
}

-(void)udpSocketDidClose:(GCDAsyncUdpSocket *)sock withError:(NSError *)error {
    STUNLog(@"STUN udpSocketDidClose=%@", error);
}
/**
